...
class GitbubAPIDidLoadTests: QuickSpec {
    override func spec() {
        let stubServices = StubServices()
        var bag: DisposeBag!
            ...
            context("try load repostirories") {
                ...
                context("succes request") {
                    ...
                    it("should return 1, based on repo number on stub") {
                        ...
                        expect(result.events).to(equal([ Recorded.next(0, 1) ]))
                    }
                }
                ...
                context("error request") {
                    ...
                    it("should return 0, request is set to false") {
                        ...
                        scheduler.start()
                        expect(result.events).to(equal([ Recorded.next(0, 0) ]))
                    }
                }
            }
        }
